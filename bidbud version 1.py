...
def save_to_excel(data, filename="bidbud_feedback_file.xlsx"):
    file_path = os.path.join("/content/", filename)
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    df = pd.DataFrame(data, columns=["Username", "Listing Number", "Date", "Feedback"])
    ...
    while True:
        ...
        for row in rows:
            try:
                ...
                counter += 1  # Increment the counter after each row is processed
...
def run_scraper():
    link = input("Enter link of feedback section of a store from Bidbud: ")
